...
async def calendar_view(
    ...
    events_by_day = await get_events_for_month(
        db, user_id=request.state.user.id, year=target_year, month=target_month
    )
...
async def handle_create_task(
    ...
    if user.role not in ("admin", "manager"):
        return RedirectResponse(
            url="/tasks?error=Only+managers+can+create+tasks", status_code=303
        )
    ...
    try:
        ...
        return RedirectResponse(
            url=f"/tasks/create?error={str(e)[:100]}", status_code=303
        )
...
async def update_task_status(
    ...
    if status in ("in_progress", "done") and task.assignee_id != user.id:
        return RedirectResponse(
            url="/tasks?error=Only+assignee+can+update+status", status_code=303
        )
...
async def join_team_form(request: Request):
    ...
    return request.app.state.templates.TemplateResponse(
        "join_team.html", {"request": request}
    )


@html_router.post("/join")
async def handle_join_team(
    request: Request, invite_code: str = Form(...), db: AsyncSession = Depends(get_db)
):
    """
    ...
    if success:
        ...
        return RedirectResponse(
            url="/join?error=Invalid+or+expired+code", status_code=303
        )
...
async def meetings_view(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    if user.team_id is not None:
        meetings = await get_user_meetings(db, user.id)
    return request.app.state.templates.TemplateResponse(
        "meetings.html", {"request": request, "user": user, "meetings": meetings}
    )
...
async def handle_create_meeting(
    ...
    try:
        ...
        return RedirectResponse(
            url="/meetings?success=Meeting+created", status_code=303
        )
    except Exception as e:
        return RedirectResponse(
            url=f"/meetings/create?error={str(e)[:100]}", status_code=303
        )


@html_router.get("/evaluations/create/{task_id}", response_class=HTMLResponse)
async def evaluation_form(
    ...
    if not task or task.status != "done":
        return RedirectResponse(
            url="/tasks?error=Only+completed+tasks+can+be+evaluated", status_code=303
        )
    return request.app.state.templates.TemplateResponse(
        "evaluation_create.html", {"request": request, "task": task}
    )
...
async def handle_evaluation(
    ...
    try:
        ...
        return RedirectResponse(
            url="/tasks?success=Evaluation+submitted", status_code=303
        )
    except Exception as e:
        return RedirectResponse(
            url=f"/evaluations/create/{task_id}?error={str(e)[:100]}", status_code=303
        )
...
async def edit_profile_form(request: Request):
    ...
    if not request.state.user:
        return RedirectResponse(url="/login")
    return request.app.state.templates.TemplateResponse(
        "profile_edit.html", {"request": request, "user": request.state.user}
    )
...
async def handle_edit_profile(
    ...
    try:
        ...
        if updated:
            return RedirectResponse(
                url="/profile/edit?success=Profile+updated", status_code=303
            )
        else:
            return RedirectResponse(
                url="/profile/edit?error=User+not+found", status_code=303
            )
...
async def delete_profile_confirm(request: Request):
    ...
    if not request.state.user:
        return RedirectResponse(url="/login")
    return request.app.state.templates.TemplateResponse(
        "profile_delete.html", {"request": request}
    )
